...
The API Worker's one-minute scheduled handler loads up to 100 pending rows with their encrypted key envelopes, provider authority, and provider source. Overlapping invocations use distinct opaque object keys; the quota transaction chooses the sole winner and every losing object is deleted. Failed R2 deletions enter the opaque Neon deletion outbox and are retried by later scheduled runs.
