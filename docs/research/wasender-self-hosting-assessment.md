...
The product permits three WhatsApp Connections per Personal Account and calls its account-wide Stored Media ceiling 5 GB; the schema fixes those values at exactly three and `5368709120` bytes, or 5 GiB, shared across the account's Connections. The private-beta admission function reserves the full three-Connection entitlement for every Personal Account rather than only its current connections. [I9]
...
| First-party availability | Over a minimum 30-day canary, the product, API, ingestion path, gateway control/data plane, auth-state authority, and Message Store meet the existing 99.5% monthly first-party availability SLO. No first-party failure is excluded because WhatsApp or another dependency was also impaired. | Monthly first-party error budget exceeded, or gateway/control failures are relabeled as provider failures to preserve the metric. [I10] |
...
- **[I9]** Personal Account connection/media limits and private-beta capacity reservation: [`CONTEXT.md`](../../CONTEXT.md), [`packages/db/migrations/0003_personal_account_bootstrap.sql`](../../packages/db/migrations/0003_personal_account_bootstrap.sql), and [`packages/db/migrations/0004_private_beta_admission.sql`](../../packages/db/migrations/0004_private_beta_admission.sql)
