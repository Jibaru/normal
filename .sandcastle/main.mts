...
const codexSandbox = () =>
  docker({
    // Copy the host gh credential into the container because macOS Keychain is
    // unavailable there. Explicit environment tokens still take precedence.
    env: {
    ...
    mounts: [
      {
        ...
        sandboxPath: "/home/agent/.codex-auth-source.json",
        readonly: true,
      },
    ],
  });
...
const hooks = {
  sandbox: {
    onSandboxReady: [
      {
        command:
          "mkdir -p ~/.codex && cp ~/.codex-auth-source.json ~/.codex/auth.json && printf 'service_tier = \"fast\"\n' > ~/.codex/config.toml && chmod 600 ~/.codex/auth.json ~/.codex/config.toml",
      },
      { command: "bun install" },
    ],
  },
...
for (let iteration = 1; iteration <= MAX_ITERATIONS; iteration++) {
  ...
  if (completedBranches.length === 0) {
    // Replanning without repository changes would select the same work again.
    console.log(
      "No commits produced. Stopping to avoid retrying unchanged work.",
    );
    break;
  }
