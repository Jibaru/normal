...
- Creates a Tool Call Log and reserves the media's full size from its integrity-protected, queryable verified plaintext-byte field before decrypting content or encrypted metadata.
...
For a new send, one Neon transaction creates the Tool Call Log, Send Operation, idempotency binding, and quota reservation and sets `status: "processing"`, `attempt_claimed_at`, and `lease_expires_at` 30 seconds later. That commit is the durable provider-attempt boundary. Before commit, failure rolls back the operation, binding, and quota and returns `isError`; after commit, quota remains consumed and every observable outcome is an operation receipt. The Worker makes one 15-second provider call. Its complete direct response is causally bound and may advance the operation without a provider message identifier; later or independently delivered webhook evidence may advance it only through ADR 0017's exact same-connection/shared-stable-identifier correlation. Lease expiry atomically changes `processing` to `unknown`; no caller can take over or redispatch it, and a concurrent exact replay returns the current receipt immediately.
