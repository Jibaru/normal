...
| `OBSERVABILITY_QUERY_URL` / `OBSERVABILITY_QUERY_TOKEN` | Secret operational endpoint and read credential | Recovery verifier only | Exact metadata-only recovery assurance query. The response is bound to the operation, guarded branch, source point, nonce, replay digest, drill start, and exact 30-day window, and includes sampled-key, isolated HMAC-rotation, and availability outcomes. RPO is derived separately from the guarded Neon branch metadata. The query must not expose request, content, provider, or tenant identifiers. |
