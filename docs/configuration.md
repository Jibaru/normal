...
| `OBSERVABILITY_QUERY_URL` / `OBSERVABILITY_QUERY_TOKEN` | Secret operational endpoint and read credential | Recovery verifier only | Exact metadata-only recovery assurance query. The response is bound to the operation, guarded branch, requested source point, nonce, replay digest, drill start, and exact 30-day window, and includes the independently observed recovered source point, sampled-key, and availability outcomes. The verifier derives RPO from the requested and independently observed points and performs isolated HMAC replacement itself instead of trusting an observability assertion. The query must not expose request, content, provider, or tenant identifiers. |
